...
- **Ask for small areas, not one big one.** `fetch_reference.py` clusters the points and queries a small box per cluster. Query cost scales with the area scanned, so one box around a whole archipelago is mostly ocean: on the 29-point test layer this cut the queried area from 158,000 km² to under 900 km². Small boxes fail independently, so a bad response costs one cluster rather than the run, and re-running retries only what failed. Small boxes are also what make the OSM core API viable as a fallback, since it has a hard 0.25 square-degree limit.
- **A search box must cover the address, not just the coordinate.** Run `verify_points.py` first. Each cluster's box spans the recorded coordinates *and* their geocodes, because a badly misplaced point's building sits near its address, not near the bad coordinate. Boxing only the recorded positions returns "no outline mapped" for precisely the points that are most wrong - the ones the audit exists to catch. An empty result then widens the box and retries up to `--max-pad`.
- **Overpass is not the only way to get OSM geometry.** When every Overpass mirror is down - which happened for hours during this skill's development - `fetch_reference.py` falls back to the OSM core API (`api.openstreetmap.org/api/0.6/map`), the service behind openstreetmap.org itself. Separate infrastructure, same data, far more reliable. It cannot filter server-side, so it returns everything in the box and the script filters locally; that is affordable only because the boxes are small. Its own limits are 0.25 square degrees and 50,000 nodes, and a dense city centre breaches the node cap even in a small box - an overflow subdivides into quarters and merges, which is the API's own documented remedy.
- **Never add a regional Overpass mirror.** National instances answer HTTP 200 with zero results outside their own extract, which is indistinguishable from "nothing is mapped here". `overpass.osm.ch` returns 3 hospitals for Zurich and 0 for Hawaii; using it as a fallback would silently report every point as having no outline. An outage is obvious and recoverable; a confident wrong answer is neither.
